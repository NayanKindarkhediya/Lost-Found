...
if (confirmDeleteBtn) {
    confirmDeleteBtn.addEventListener("click", async function () {
        // deleteIndex html file se aa raha hai
        if (typeof deleteIndex !== 'undefined' && deleteIndex !== null) {
            try {
                // 1. Database se items fetch karke match karenge ki kaunsa delete karna hai
                let res = await fetch('http://localhost:5000/get-items/lost');
                let items = await res.json();
                
                // Array index se item dhundho
                let itemToDelete = items[deleteIndex];
                
                // 2. Agar database ID mil gayi, toh backend se delete request bhejo
                if (itemToDelete && itemToDelete.id) {
                    await fetch(`http://localhost:5000/delete-item/${itemToDelete.id}`, {
                        method: 'DELETE'
                    });
                } else {
                    // Agar local storage use ho raha hai (fallback)
                    let lostItems = JSON.parse(localStorage.getItem("lostItems")) || [];
                    lostItems.splice(deleteIndex, 1);
                    localStorage.setItem("lostItems", JSON.stringify(lostItems));
                }

                // 3. Modal close karo aur list refresh karo
                if (typeof closeDeleteModal === "function") closeDeleteModal();
                if (typeof fetchItemsFromDB === "function") fetchItemsFromDB(); 
